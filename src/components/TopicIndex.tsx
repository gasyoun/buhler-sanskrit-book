...
const TopicIndex: React.FC = () => {
    ...
    const filtered = useMemo(() => {
        return allEntries.filter(entry => {
            ...
            const matchesText = !filter || entry.topic.toLowerCase().includes(filter.toLowerCase());
        ...
        <div>
            <div style={{display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap'}}>
                ...
                >
                    ...
                    {lessons.map(l => (
                        <option key={l} value={l}>Урок {l}</option>
                    ))}
                </select>
            </div>

            {grouped.length === 0 && (
                <p style={{color: 'var(--ifm-color-emphasis-600)'}}>Ничего не найдено.</p>
            )}

            {grouped.map(([lesson, entries]) => (
                <div key={lesson} style={{marginBottom: '1.5rem'}}>
                    <h3>
                        <a href={`${docsBase}/lesson${lesson}`} style={{color: 'var(--ifm-color-primary)'}}>
                            Урок {lesson}
                        </a>
                    </h3>
                        ...
                        {entries.map(entry => (
                            <li key={entry.id} style={{marginBottom: '0.4rem'}}>
                                <a
                                    ...
                                    style={{color: 'var(--ifm-font-color-base)', textDecoration: 'none'}}
                                >
                                    {entry.topic}
                                </a>
                            </li>
