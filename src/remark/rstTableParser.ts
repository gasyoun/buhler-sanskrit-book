export interface RstCell {
  text: string;
  rowSpan: number;
  colSpan: number;
  header: boolean;
}
export type RstRow = RstCell[];
...
export function parseRstGridTable(source: string): RstTableModel {
  ...
  const headerRows: RstRow[] = [];
  const bodyRows: RstRow[] = [];
  for (let r = 0; r < R; r++) {
    const row: RstRow = [];
    for (let c = 0; c < C; c++) {
